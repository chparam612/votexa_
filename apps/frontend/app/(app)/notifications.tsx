      ...
      <ScrollView className="flex-1 p-6">
        <View className="bg-white p-6 rounded-2xl shadow-sm mb-8">
          ...
          <View className="flex-row justify-between items-center mb-4">
            ...
            <Switch value={prefs.push} onValueChange={() => togglePref('push')} trackColor={{ true: '#3B82F6' }} />
          </View>
          <View className="flex-row justify-between items-center mb-4">
            <Text className="text-slate-700">Email Alerts</Text>
            <Switch value={prefs.email} onValueChange={() => togglePref('email')} trackColor={{ true: '#3B82F6' }} />
          </View>
          <View className="flex-row justify-between items-center mb-4">
            <Text className="text-slate-700">SMS Reminders</Text>
            <Switch value={prefs.sms} onValueChange={() => togglePref('sms')} trackColor={{ true: '#3B82F6' }} />
          </View>

          <TouchableOpacity onPress={testPush} className="bg-slate-100 p-3 rounded-xl items-center mt-2">
        ...
        <View className="space-y-3 pb-12">
          {notifications.map(n => (
            <View key={n.id} className={`bg-white p-4 rounded-xl border-l-4 ${n.read ? 'border-l-slate-300' : 'border-l-blue-500 bg-blue-50'}`}>
              <Text className="font-bold text-slate-900">{n.title}</Text>
              <Text className="text-slate-600 mt-1">{n.body}</Text>
              <Text className="text-slate-400 text-xs mt-2">{new Date(n.timestamp).toLocaleString()}</Text>
            </View>
          ))}
