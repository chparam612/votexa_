  ...
  return (
    ...
    >
      <View className="flex-row justify-between items-start mb-2">
        ...
        >
          <Text className="text-blue-900 font-bold text-lg" maxFontSizeMultiplier={1.3}>{score}</Text>
          <Text className="text-blue-600 text-xs" maxFontSizeMultiplier={1.3}>Score</Text>
        </View>
      </View>
      ...
      <View className="flex-row flex-wrap gap-2 mb-4 mt-2">
        ...
        >
          <Text className={`text-xs font-bold ${crowdBadge.split(' ')[1]}`} maxFontSizeMultiplier={1.3}>{crowdText}</Text>
        </View>
        <View
          className="bg-slate-100 px-2 py-1 rounded-md"
          accessibilityLabel={`Estimated wait time: ${avg_wait_minutes} minutes`}
        >
          <Text className="text-xs font-bold text-slate-700" maxFontSizeMultiplier={1.3}>~{avg_wait_minutes}m wait</Text>
        </View>
        <View
          className="bg-slate-100 px-2 py-1 rounded-md"
          accessibilityLabel={`Distance: ${distance_km.toFixed(1)} kilometers`}
        >
          <Text className="text-xs font-bold text-slate-700" maxFontSizeMultiplier={1.3}>{distance_km.toFixed(1)} km away</Text>
        </View>
      </View>

      <Pressable
        className={(({ pressed }: any) => 
          `p-3 rounded-xl items-center ${pressed ? 'bg-blue-800' : 'bg-blue-600'}`
        ) as any}
        onPress={openMaps}
      ...
      >
        <Text className="text-white font-bold" maxFontSizeMultiplier={1.3}>Get Directions</Text>
      </Pressable>
    </View>
  );
}
