...
const STATES_ORDER: VoterState[] = [
  'NOT_REGISTERED', 'REGISTERED', 'VERIFIED', 'READY', 'VOTED'
];
  ...
  if (currentState === 'DISQUALIFIED') {
    return (
      <View className="bg-red-50 p-4 rounded-xl border border-red-200 items-center">
        ...
        <Text className="text-red-500 text-center mt-1">Please contact your local election officer for assistance.</Text>
      </View>
    );
  }
  ...
  return (
    <View className="bg-white p-4 rounded-2xl shadow-sm">
      {STATES_ORDER.map((state, index) => {
        ...
        return (
          <View key={state} className="flex-row items-start mb-4 last:mb-0">
            ...
            <View className="flex-1 pt-1">
              <Text className={`font-bold text-base ${isCurrent ? 'text-blue-900' : isCompleted ? 'text-green-800' : 'text-slate-400'}`}>
                {meta.label}
              </Text>
            </View>
          </View>
